...
<section id="produk" class="py-16 md:py-24 px-3 md:px-6 bg-[#0a0a0a]">
    <div class="max-w-7xl mx-auto">
        ...
        <!-- Products Grid (2 columns on mobile like Shopee) -->
        <div class="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-8">
            @foreach($products as $product)
            <div class="card-product group flex flex-col" id="product-{{ $product->id }}">
                <!-- Product Image -->
                ...
                <div class="p-3 md:p-6 relative z-10 flex flex-col flex-grow">
                    <h3 class="text-[13px] md:text-xl font-bold text-[#d4af37] mb-1 md:mb-2 line-clamp-2" style="font-family:'Cormorant Garamond',serif; line-height: 1.3;">{{ $product->name }}</h3>
                    <p class="text-[10px] md:text-sm text-[#f3e9c6]/60 leading-relaxed mb-3 md:mb-6 line-clamp-1 md:line-clamp-2">{{ $product->description }}</p>
                    <div class="mt-auto">
                        <button
                            onclick="openOrderModal({{ $product->id }}, '{{ addslashes($product->name) }}', {{ $product->price }}, '{{ $product->formatted_price }}')"
                            class="btn-gold w-full py-2 md:py-3.5 text-[9px] md:text-[11px] shadow-lg shadow-[#d4af37]/20 rounded-md md:rounded-xl">
